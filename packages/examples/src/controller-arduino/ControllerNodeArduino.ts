...
class ControllerNode {
    async start() {
        ...
        if (environment.vars.get("ble")) {
            ...
            const threadNetworkName = environment.vars.string("blethreadnetworkname");
            const threadOperationalDataset = environment.vars.string("blethreadoperationaldataset");
        ...
        try {
            ...
            const devices = node.getDeviceById(3);

            if (devices && devices.number === 3) {
                ...
                const onOff: ClusterClientObj<OnOff.Complete> | undefined = devices.getClusterClient(OnOff.Complete);
                if (onOff !== undefined) {
